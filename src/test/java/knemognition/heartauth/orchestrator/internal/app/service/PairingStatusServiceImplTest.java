...
class PairingStatusServiceImplTest extends HeartauthUnitTest {

    @Mock
    StatusStore<PairingState> store;
    @Mock
    ResponseStatusMapper mapper;
    @InjectMocks
    PairingStatusServiceImpl service;

    @Test
    void status_returns_notFound_when_absent() {
        UUID id = UUID.randomUUID();
        StatusResponse notFound = new StatusResponse().status(FlowStatus.NOT_FOUND);
